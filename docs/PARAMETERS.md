...
| `fetch_timeout_seconds` | `600` | How long a single yt-dlp call may take before Winnow stops waiting. **Not measured** — no experiment here establishes a typical fetch, and a video download depends entirely on the file and the link. It is there to bound a hang, not to be tight: without it a stalled connection leaves Winnow waiting with no output and no end. |
...
| `frame_timeout_seconds` | `300` | How long ffmpeg may take to sample frames from one file, used only when the pack sets `use_frames`. **Not measured** — sampling decodes through the file, so a three-hour recording is minutes and a short talk is seconds. It bounds a hang (a truncated download, a container ffmpeg cannot read), and a timeout skips the frames with a message rather than failing the ingest. |
